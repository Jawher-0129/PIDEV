...
class User1Repository extends ServiceEntityRepository implements PasswordUpgraderInterface
{
    ...
    // Méthode pour rechercher les utilisateurs par nom
  // src/Repository/User1Repository.php
  ...
  public function findBySearchTerm($searchTerm)
  {
      ...
      if (!empty($searchTerm)) {
          $qb->where('u.Nom LIKE :searchTerm')
              ...
              ->orWhere('u.Adresse LIKE :searchTerm')
...
{
    return $this->createQueryBuilder('u')
        ->select('COUNT(u) as userCount, u.roles')
        ->groupBy('u.roles')
        ->getQuery()
        ->getResult();
}
